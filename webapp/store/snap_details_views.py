...
def charm_details_views(store, api, handle_errors):
    charm_regex = "[a-z0-9-]*[a-z][a-z0-9-]*"
    charm_regex_upercase = "[A-Za-z0-9-]*[A-Za-z][A-Za-z0-9-]*"

    def _get_context_charm_details(charm_name):
        try:
            details = api.get_item_details(charm_name, api_version=2)
            ...
            if api_response_error_list.status_code == 404:
                flask.abort(404, "No charm named {}".format(charm_name))
            else:
        ...
        context = {
            "entity-id": details.get("snap-id"),
            # Data direct from details API
            "entity_title": details["snap"]["title"],
    ...
    @store.route('/<regex("' + charm_regex + '"):charm_name>')
    def charm_details(charm_name):
        """
        ...
        context = _get_context_charm_details(charm_name)
    ...
    @store.route('/<regex("' + charm_regex_upercase + '"):charm_name>')
    def snap_details_case_sensitive(charm_name):
        return flask.redirect(
            flask.url_for(".charm_details", charm_name=charm_name.lower())
        )
